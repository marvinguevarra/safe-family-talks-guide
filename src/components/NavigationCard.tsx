...
export const NavigationLink = ({
  ...
  return (
    <Link 
      to={to} 
      className="flex items-center justify-between p-3 bg-white/90 dark:bg-black/60 rounded-[15px] hover:bg-white dark:hover:bg-black/80 transition-all duration-200 group text-foreground"
    >
      <div className="flex items-center gap-3">
        <span className="font-semibold text-foreground">{children}</span>
        {ageGroup && (
          <span className="text-xs bg-black/20 dark:bg-white/20 text-foreground px-2 py-1 rounded-full font-medium">
            {ageGroup}
          </span>
        )}
      </div>
